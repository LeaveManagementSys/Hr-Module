...
public class HrServiceImpl implements HrService {
	...
	public String employeeInsert(InsertEmployeeModel insertemployeemodel) {		
		...
		String result="fail";
		if(!employeeFound) {			
			if(insertemployeemodel.getSalary()>25000) {
				employee.setSalary(insertemployeemodel.getSalary());
			}else {
				try {
	...
	public String salaryupdate(UpdateEmployeeSalaryModel updateempsalarymodel) {
		...
		String result="fail";
		for(RetriveEmployeeModel retriveempmodel: employeesList) {
			if( retriveempmodel.getEmployeeId()==updateempsalarymodel.getEmployeeId()) {
				employee.setEmployeeId(updateempsalarymodel.getEmployeeId());
				employeeFound=true;
				break;
			}
		}		
         if(employeeFound) {
        		...
        		if(updateempsalarymodel.getNewSalary()>25000) {
        			employee.setSalary(updateempsalarymodel.getNewSalary());
        		}else {
        			try {
        			throw new InvalidSalaryException("Salary not valid");
        			}catch(InvalidSalaryException e) {
	...
	public String deleteEmployee(RetriveEmployeeModel retriveemployeemodel) {
		...
		Employee employee=new Employee();
		for(RetriveEmployeeModel retriveempmodel: employeesList) {
			if( retriveempmodel.getEmployeeId()==retriveemployeemodel.getEmployeeId()) {
				employee.setEmployeeId(retriveemployeemodel.getEmployeeId());
				employeeFound=true;
				break;
			}
		}
	...
	public String phoneNumberUpdate(UpdateEmpPhoneNumberModel updateempphonenumbermodel) {
		...
		Employee employee=new Employee();
		for(RetriveEmployeeModel retriveempmodel: employeesList) {
			if( retriveempmodel.getEmployeeId()==updateempphonenumbermodel.getEmployeeId()) {
				employee.setEmployeeId(updateempphonenumbermodel.getEmployeeId());
				employeeFound=true;
				break;
			}
		}
	...
	public String designationUpdate(UpdateEmployeeDesignationModel updateemployeedesgmodel) {	
		...
		for(RetriveEmployeeModel retriveempmodel: employeesList) {
			if( retriveempmodel.getEmployeeId()==updateemployeedesgmodel.getEmployeeId()) {
				employee.setEmployeeId(updateemployeedesgmodel.getEmployeeId());
				employeeFound=true;
				break;
			}
		}
		...
		List<RetriveEmployeeDesignationModel> jobsTitleList=retrieveDesignation();
		for(RetriveEmployeeDesignationModel retriveEmployeeModelDesignation:jobsTitleList ) {
			if(retriveEmployeeModelDesignation.getDesignation().equalsIgnoreCase(updateemployeedesgmodel.getUpdateDesignation())){
				employee.setDesignation(updateemployeedesgmodel.getUpdateDesignation());
				designationFound=true;
				break;
			}
		}
